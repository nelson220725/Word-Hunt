...
A feature-rich OCaml implementation of the popular Game Pigeon Word Hunt game, featuring an interactive GUI, intelligent board solving, and multiple game modes.
...
#### `BuildBoard` (src/builder.ml)
- Generates random 4x4 letter boards with realistic letter frequency
- Validates word formation based on tile adjacency
- Implements board solver using backtracking algorithm
- Finds and ranks all possible words on any board

#### `Trie` (src/data_structures.ml)
- Efficient prefix tree for word storage and lookup
- Enables fast word validation during gameplay
- Optimized for Scrabble dictionary size

#### `Dictionary` (src/data_structures.ml)
- Scrabble dictionary interface
- Provides word validation functionality
- Supports board solver with comprehensive word list

### Key Algorithms

1. **Board Solver**: DFS-based traversal that explores all possible word paths
2. **Word Validation**: Combines Trie lookup with adjacency and uniqueness checks
3. **Letter Generation**: Weighted random selection based on English letter frequency

## Development

### Available Make Targets
- `make build` - Compile the project
- `make GUI` - Run graphical interface
- `make game` - Run command-line interface
- `make test` - Run test suite
- `make utop` - Start OCaml REPL with project loaded
- `make bisect` - Generate code coverage report
